...
test('v0.6.14 root is a public Google-verification-ready Appointment Lite homepage', async t => {
...
test('v0.6.14 keeps SHOPLINE install entry behavior on root query parameters', async t => {
  const base = await withServer(t);
...
test('v0.6.14 exposes bilingual public terms and a Chinese product homepage', async t => {
  const base = await withServer(t);
...
test('v0.6.14 terms shortcut follows browser language', async t => {
  const base = await withServer(t);
...
test('v0.6.14 public site exposes crawler metadata and linked legal navigation', async t => {
  const base = await withServer(t);
...
test('v0.6.14 homepage accurately describes current owned-calendar scope behavior', () => {
...
test('v0.6.14 release documentation lists the production Google Auth Platform values', async () => {

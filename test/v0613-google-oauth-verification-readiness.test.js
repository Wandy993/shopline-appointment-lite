...
test('v0.6.13 root is a public Google-verification-ready Appointment Lite homepage', async t => {
...
test('v0.6.13 keeps SHOPLINE install entry behavior on root query parameters', async t => {
  const base = await withServer(t);
...
test('v0.6.13 exposes bilingual public terms and a Chinese product homepage', async t => {
  const base = await withServer(t);
...
test('v0.6.13 terms shortcut follows browser language', async t => {
  const base = await withServer(t);
...
test('v0.6.13 public site exposes crawler metadata and linked legal navigation', async t => {
  const base = await withServer(t);
...
test('v0.6.13 homepage accurately describes current owned-calendar scope behavior', () => {
...
test('v0.6.13 release documentation lists the production Google Auth Platform values', async () => {

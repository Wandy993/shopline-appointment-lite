...
test('v0.8.1.1 admin cache and health build identify the wizard polish release', async () => {
  ...
  assert.match(view, /styles\.css\?v=0\.8\.1&build=0\.8\.1\.1/);
  assert.match(view, /app\.js\?v=0\.8\.1&build=0\.8\.1\.1/);
  assert.match(app, /build: '0\.8\.1\.1-service-wizard-simplification-ui-polish\.1'/);
  assert.match(app, /release: 'v0\.8\.1\.1-service-wizard-simplification-ui-polish'/);
  assert.match(release, /RELEASE_LABEL="0\.8\.1\.1"/);

...
test('v0.7.0.2 keeps active and grace-period subscriptions in full mode', () => {
...
test('v0.7.0.2 moves previously activated stores into archive mode after grace ends', () => {
  const now = new Date('2026-08-28T12:00:00.000Z');
  const ended = {
    status: 'expired',
...
test('v0.7.0.2 still gates stores that never had an active subscription', () => {
  const now = new Date('2026-08-28T12:00:00.000Z');
...
test('v0.7.0.2 server only exposes booking reads during archive mode', async () => {
...
test('v0.7.0.2 admin archive UI keeps booking export and billing renewal while removing booking actions', async () => {
...
test('v0.7.0.2 suppresses runtime booking operations while the subscription is inactive', async () => {

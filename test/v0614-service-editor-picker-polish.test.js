...
test('v0.6.15 service wizard replaces native select presentation with Appointment Lite pickers', async () => {
  ...
  assert.match(css, /v0\.6\.15 — Service editor picker polish/);
...
test('v0.6.15 dynamic availability exception selects inherit the custom picker layer', async () => {
...
test('v0.6.15 picker menus flip inside the visible wizard and close during scrolling', async () => {
...
test('v0.6.15 service time zone uses a searchable Appointment Lite picker instead of browser datalist UI', async () => {
  const [view, app, css] = await Promise.all([
    source('src/views/admin.js'),
    source('public/admin/app.js'),
    source('public/admin/styles.css')
...
test('v0.6.15 SHOPLINE location picker gains the same viewport-aware drop-up safety', async () => {
  const [app, css] = await Promise.all([source('public/admin/app.js'), source('public/admin/styles.css')]);
...
test('v0.6.15 paid checkout guidance has deliberate spacing before checkout controls', async () => {
...
test('v0.6.15 release version stays aligned', async () => {
  ...
  assert.match(legalView, /styles\.css\?v=0\.6\.15/);

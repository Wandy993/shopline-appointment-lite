...
test('v0.6.14 service wizard replaces native select presentation with Appointment Lite pickers', async () => {
  ...
  assert.match(css, /v0\.6\.14 — Service editor picker polish/);
...
test('v0.6.14 dynamic availability exception selects inherit the custom picker layer', async () => {
...
test('v0.6.14 picker menus flip inside the visible wizard and close during scrolling', async () => {
...
test('v0.6.14 service time zone uses a searchable Appointment Lite picker instead of browser datalist UI', async () => {
  const [view, app, css] = await Promise.all([
    source('src/views/admin.js'),
    source('public/admin/app.js'),
    source('public/admin/styles.css')
...
test('v0.6.14 SHOPLINE location picker gains the same viewport-aware drop-up safety', async () => {
  const [app, css] = await Promise.all([source('public/admin/app.js'), source('public/admin/styles.css')]);
...
test('v0.6.14 paid checkout guidance has deliberate spacing before checkout controls', async () => {
...
test('v0.6.14 release version stays aligned', async () => {

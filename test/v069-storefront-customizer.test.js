...
test('v0.6.9 release versions stay aligned', async () => {
  ...
  assert.equal(JSON.parse(pkgText).version, '0.6.11');
  assert.match(adminView, /styles\.css\?v=0.6.11/);
  assert.match(adminView, /app\.js\?v=0.6.11/);
  assert.match(bookView, /styles\.css\?v=0.6.11/);
  assert.match(bookView, /app\.js\?v=0.6.11/);
  assert.match(theme, /const VERSION = '0.6.11'/);
  assert.match(health, /version: '0.6.11'/);

...
<body>
    ...
    <div class="workspace">
      ...
      <main class="content">
        ...
        <section id="rulesView" class="view hidden">
          <div class="page-heading"><div><span class="eyebrow">SERVICE CATALOG</span><h1>Services & appointment rules</h1><p>Create product appointments, in-store visits, home services, consultations, classes, and shareable booking experiences.</p></div><button class="primary" data-new-rule>${icons.plus} New service</button></div>
        ...
        <section id="bookingsView" class="view hidden">
          ...
          <div class="booking-filter-panel panel">
            <div class="booking-filter-row"><label class="search-field">${icons.search}<input id="bookingSearch" type="search" placeholder="Search customer, service, or email"></label><label class="compact-select"><span>Service</span><select id="bookingServiceFilter"><option value="">All services</option></select></label><label class="compact-select"><span>Status</span><select id="bookingStatusFilter"><option value="">All statuses</option><option value="confirmed">Confirmed</option><option value="completed">Completed</option><option value="no_show">No-show</option><option value="cancelled">Cancelled</option></select></label><div class="segmented view-switch" role="group" aria-label="Booking view"><button class="active" data-booking-view="list">List</button><button data-booking-view="calendar">Calendar</button></div></div>
            <div class="booking-filter-row secondary-filters"><label class="compact-date"><span>From</span><input id="bookingFrom" type="date"></label><label class="compact-date"><span>To</span><input id="bookingTo" type="date"></label><button id="clearBookingFilters" class="text-button" type="button">Clear filters</button><span id="bookingResultCount" class="filter-count"></span></div>
          </div>
        ...
        <section id="setupView" class="view hidden">
          <div class="page-heading"><div><span class="eyebrow">QUICK SETUP</span><h1>Launch Appointment Lite</h1><p>Connect the App Block for product bookings, or create a standalone service, then test the booking flow.</p></div><span class="status-badge success">Store connected</span></div>
          <div class="setup-layout">
            <article class="panel setup-steps">
              <div id="setupBlockStep" class="setup-step"><span>1</span><div><strong>Enable the Appointment Lite App Block</strong><p>Required for SHOPLINE product appointments. Open the product template in the SHOPLINE theme editor, add or activate the Appointment Lite App Block, then save the theme. Standalone services can continue without it.</p><div class="setup-step-actions"><a id="openThemeEditor" class="button-link primary disabled" href="#" target="_blank" rel="noopener noreferrer">Open theme editor ${icons.arrow}</a><button id="confirmAppBlock" class="secondary" type="button">I've enabled the App Block</button></div><p id="themeEditorHint" class="hint">Preparing your theme editor link…</p></div></div>
              <div id="setupServiceStep" class="setup-step"><span>2</span><div><strong>Create your first appointment service</strong><p>Choose a product appointment or standalone service, then configure duration, availability, location, and specialist.</p><button class="text-button" data-new-rule>Create a service ${icons.arrow}</button></div></div>
              <div id="setupTestStep" class="setup-step"><span>3</span><div><strong>Test the storefront booking flow</strong><p>Open the configured product page or standalone booking link and submit one test booking. The booking should appear in Bookings.</p><a id="setupPreviewProduct" class="button-link secondary-link disabled" href="#" target="_blank" rel="noopener noreferrer">Open booking experience ${icons.arrow}</a></div></div>
            </article>
            <aside class="panel diagnostics"><div class="diagnostic-icon">${icons.check}</div><h2>Quick setup</h2><p>Product appointments use the App Block. In-store, onsite, consultation, class, and other standalone services can be created without editing the theme.</p><span id="setupStoreId" class="setup-meta">Loading store…</span><ul><li>Step 1 connects product appointments to the storefront App Block</li><li>Step 2 creates the first appointment service</li><li>Step 3 verifies the complete customer booking experience</li></ul></aside>
          </div>
        </section>
      </main>
    </div>
  ...
  <dialog id="ruleDialog" class="modal rule-modal">
    <form id="ruleForm">
      ...
      <div class="modal-body">
        <input type="hidden" id="ruleId"><input type="hidden" id="productSelect"><input type="hidden" id="serviceType" value="product"><input type="hidden" id="sourceType" value="product">
        <section class="rule-step" data-rule-step="0">
          <div class="step-intro"><h3>What kind of service is this?</h3><p>Product appointments use your SHOPLINE product page. Other services get a standalone booking link you can share anywhere.</p></div>
          <div id="serviceTypeGrid" class="service-type-grid">
            <button type="button" class="service-type-option selected" data-service-type="product"><strong>Product booking</strong><span>Appointments connected to a SHOPLINE product and App Block.</span></button>
            ...
            <button type="button" class="service-type-option" data-service-type="other"><strong>Other service</strong><span>Create a flexible standalone appointment service.</span></button>
          </div>
          <div id="productSourceFields"><div class="field"><label>SHOPLINE product</label><div id="productPicker" class="custom-select"><button id="productPickerButton" type="button" aria-haspopup="dialog"><span id="productPickerLabel">Select a product</span>${icons.chevron}</button></div><p class="hint">Each product can have one active appointment service.</p></div></div>
          <div id="standaloneSourceFields" class="hidden"><div class="field"><label for="serviceTitle">Service name</label><input id="serviceTitle" maxlength="255" placeholder="e.g. Home installation service"><p class="hint">A shareable booking link is created automatically after you save.</p></div></div>
  ...
  <dialog id="quickstartDialog" class="modal quickstart-modal">
    ...
    <div class="modal-body quickstart-body">
      <div class="quickstart-progress"><div><strong id="quickstartProgressLabel">0 of 3 complete</strong><span>The App Block is required for product appointments. Standalone services can continue directly to Step 2.</span></div><div class="progress-track"><span id="quickstartProgress"></span></div></div>
      <div class="quickstart-steps">
        <article id="quickstartBlockStep" class="quickstart-step"><span class="quickstart-number" data-step="1">1</span><div><strong>Enable the Appointment Lite App Block</strong><p>For product appointments, open the product template, activate the Appointment Lite App Block, and save the theme. Standalone services can skip this step.</p><div class="setup-step-actions"><a id="quickstartThemeEditor" class="button-link primary disabled" href="#" target="_blank" rel="noopener noreferrer">Open theme editor ${icons.arrow}</a><button id="quickstartConfirmBlock" class="secondary" type="button">I've enabled the App Block</button></div></div></article>
        <article id="quickstartServiceStep" class="quickstart-step"><span class="quickstart-number" data-step="2">2</span><div><strong>Create your first appointment service</strong><p>Choose a product appointment or standalone service and configure the schedule.</p><button id="quickstartCreateService" class="secondary" type="button">Create a service ${icons.arrow}</button></div></article>

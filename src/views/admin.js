...
<body>
  <div class="app-shell">
    <aside class="sidebar">
      <div class="brand">${brand()}<div><strong>Appointment Lite</strong><span>Service workspace</span></div></div>
      ...
      <div class="sidebar-status"><span class="pulse"></span><div><strong>System online</strong><span id="sidebarProvider">Checking email…</span></div></div>
    </aside>
      ...
      <main class="content">
        ...
        <section id="dashboardView" class="view">
          <div class="hero-panel">
            <div class="hero-copy"><span class="eyebrow">APPOINTMENT OPERATIONS</span><h1>One calm place to run every consultation.</h1><p>Configure bookable products, stay ahead of upcoming visits, and keep customer communication on brand.</p><div class="hero-actions"><button class="primary" data-new-rule>${icons.plus} Create service rule</button><button class="ghost" data-go-view="bookings">View bookings ${icons.arrow}</button></div></div>
          ...
          <div class="stats">
            ...
            <article><div class="stat-icon amber">${icons.spark}</div><div><span>Rule access</span><strong id="planName">—</strong><small>MVP workspace</small></div></article>
          </div>
        ...
        <section id="bookingsView" class="view hidden">
          ...
          <div class="booking-table panel"><div class="table-head"><span>Customer & service</span><span>Date & time</span><span>Assignment</span><span>Status</span><span></span></div><div id="bookingsList"><div class="loading">Loading bookings…</div></div></div>
        </section>
        ...
        <section id="emailView" class="view hidden">
          ...
          <div class="email-status-bar"><span id="emailStatusDot" class="status-dot"></span><div><strong id="emailStatusTitle">Checking delivery provider…</strong><span id="emailStatusText"></span></div><span id="emailFromText" class="provider-chip"></span></div>
          <div class="email-studio-grid">
            <div class="email-controls">
              <article class="panel form-section"><div class="section-title"><span class="section-number">01</span><div><h2>Brand identity</h2><p>Used inside the email header. Inbox avatars are managed separately by mailbox providers.</p></div></div><div class="field-row"><div class="field"><label for="emailBrandName">Brand name</label><input id="emailBrandName" maxlength="80" placeholder="Your store name"></div><div class="field color-field"><label for="emailAccentColor">Accent color</label><div><input id="emailAccentColor" type="color" value="#5B5BD6"><input id="emailAccentHex" maxlength="7" value="#5B5BD6" aria-label="Accent color hex"></div></div></div><div class="field"><label for="emailLogoUrl">Email logo URL <span>optional</span></label><input id="emailLogoUrl" type="url" maxlength="500" placeholder="https://cdn.example.com/logo.png"><p class="hint">Use a square HTTPS image, ideally 160 × 160 px. If empty, a branded initial is shown.</p></div><div class="avatar-note"><div class="mini-avatar">A</div><p><strong>About the Gmail “A” avatar</strong><span>HTML controls the logo inside your email, not the sender avatar in Gmail. That avatar normally comes from the sending Google account or BIMI/verified-domain setup.</span></p></div></article>
              <article class="panel form-section"><div class="section-title"><span class="section-number">02</span><div><h2>Delivery routing</h2><p>Credentials stay safely in Railway; these settings can differ per store.</p></div></div><div class="field-row"><div class="field"><label for="emailReplyTo">Customer reply-to</label><input id="emailReplyTo" type="email" maxlength="254" placeholder="support@yourstore.com"></div><div class="field"><label for="merchantNotificationEmail">New booking notifications</label><input id="merchantNotificationEmail" type="email" maxlength="254" placeholder="appointments@yourstore.com"></div></div></article>
        ...
        <section id="setupView" class="view hidden">
          <div class="page-heading"><div><span class="eyebrow">STOREFRONT CONNECTION</span><h1>Storefront setup</h1><p>A guided checklist for connecting Appointment Lite to your product pages.</p></div><span class="status-badge success">Backend connected</span></div>
          <div class="setup-layout">
            <article class="panel setup-steps"><div class="setup-step completed"><span>1</span><div><strong>Connect the SHOPLINE store</strong><p>OAuth authorization and store metadata are active.</p><code id="setupStoreId">Store ID loading…</code></div></div><div class="setup-step"><span>2</span><div><strong>Create a service rule</strong><p>Select a product, appointment duration, availability, location, and staff label.</p><button class="text-button" data-new-rule>Create a rule ${icons.arrow}</button></div></div><div class="setup-step"><span>3</span><div><strong>Push the Theme App Extension</strong><p>Copy the supplied zero-configuration source into the CLI-created extension and push it.</p><code>rsync -a theme-extension-source/ theme-app-extension/</code></div></div><div class="setup-step"><span>4</span><div><strong>Add the App Block</strong><p>Add Appointment Lite to the product template. The block itself has no settings—it acts only as an on/off switch.</p></div></div></article>
            <aside class="panel diagnostics"><div class="diagnostic-icon">${icons.spark}</div><h2>Theme diagnostics</h2><p>In storefront preview, open the browser console and filter for:</p><code>[Appointment Lite]</code><ul><li>Store and product identity</li><li>Rule cache and API status</li><li>Block visibility decisions</li><li>Availability and booking requests</li></ul></aside>
          </div>
        </section>
      </main>
    </div>

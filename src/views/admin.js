...
<head>
  ...
  <link rel="stylesheet" href="/admin/styles.css?v=0.8.1">
</head>
<body>
  <div class="app-shell">
    <aside class="sidebar">
    ...
    <form id="ruleForm" novalidate>
      ...
      <div class="modal-body">
        ...
        <section class="rule-step" data-rule-step="0">
          ...
          <fieldset class="choice-fieldset booking-business-fieldset"><legend>How is this appointment unlocked?</legend><p class="fieldset-hint">Keep the booking business model separate from where the booking widget appears in your storefront.</p><div id="bookingTypeGrid" class="booking-type-grid">
            <button type="button" class="booking-type-option selected" data-booking-type="standalone"><strong>Standalone booking</strong><span>Customers can book without first purchasing a specific SHOPLINE product.</span><small>Consultations, classes, showroom visits, sales meetings, and independent services.</small></button>
            <button type="button" class="booking-type-option" data-booking-type="purchase_triggered"><strong>Purchase-triggered booking</strong><span>Customers book only after an eligible SHOPLINE product is paid.</span><small>Installation, onboarding, delivery setup, paid service packages, and post-purchase fulfillment.</small></button>
          </div></fieldset>
          <fieldset id="standalonePaymentFieldset" class="choice-fieldset"><legend>Does this standalone appointment require payment?</legend><div id="paymentModeGrid" class="payment-mode-grid">
            <button type="button" class="payment-mode-option selected" data-payment-mode="none"><strong>No payment</strong><span>The appointment is confirmed immediately after the customer selects a time.</span></button>
            <button type="button" class="payment-mode-option" data-payment-mode="checkout"><strong>Pay during booking</strong><span>Choose a time first, then complete SHOPLINE checkout before confirmation.</span></button>
          </div></fieldset>
          <div id="purchaseTriggerFields" class="booking-model-panel hidden"><div class="panel-copy"><span class="eyebrow">PURCHASE ELIGIBILITY</span><strong>Products that unlock this appointment</strong><p>Paid orders containing any selected product create appointment eligibility. These products do not control where the booking widget is displayed.</p></div><button id="triggerProductPickerButton" type="button" class="secondary">Choose trigger products</button><div id="triggerProductSummary" class="selected-product-chips"></div></div>
          <div id="checkoutProductFields" class="booking-model-panel hidden"><div class="panel-copy"><span class="eyebrow">BOOKING CHECKOUT</span><strong>Checkout product</strong><p>This product only supplies SHOPLINE price and checkout for this appointment. It is not a storefront placement setting.</p></div><div class="field"><label>SHOPLINE checkout product</label><div id="productPicker" class="custom-select"><button id="productPickerButton" type="button" aria-haspopup="dialog"><span id="productPickerLabel">Select checkout product</span>${icons.chevron}</button></div></div><div id="paidCheckoutFields" class="paid-checkout-fields"><div class="field-row"><div class="field"><label for="paidVariantSelect">Checkout variant</label><select id="paidVariantSelect"><option value="">Select a product first</option></select></div><div class="field"><label for="paymentHoldMinutes">Payment hold</label><select id="paymentHoldMinutes"><option value="5">5 minutes</option><option value="10">10 minutes</option><option value="15" selected>15 minutes</option><option value="20">20 minutes</option><option value="30">30 minutes</option></select></div></div><p id="paidCheckoutVariantMeta" class="hint">Select the SHOPLINE product used to charge for this appointment.</p></div></div>
          <div id="purchaseTriggeredNotice" class="post-purchase-entry-card hidden"><span class="post-purchase-entry-icon">${icons.email}</span><div class="post-purchase-entry-copy"><span class="post-purchase-entry-kicker">AFTER PAYMENT</span><strong>Private scheduling entitlement</strong><p>Appointment Lite waits for a paid eligible order, then sends the buyer a private scheduling link.</p><div class="post-purchase-entry-meta"><span>1 eligible unit = 1 appointment</span><span>Storefront placement is not used to bypass purchase eligibility</span></div></div></div>
  ...
  <script type="module" src="/admin/app.js?v=0.8.1"></script>
</body>
</html>`;
}
